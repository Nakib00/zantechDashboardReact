...
const ViewProduct = () => {
  ...
  const handleToggleStatus = useCallback(async () => {
    ...
    try {
      ...
      setProduct(prev => ({ ...prev, status: prev.status === "1" ? "0" : "1" }));
  ...
  const handleQuickEditSubmit = useCallback(async (e) => {
    e.preventDefault();
    setEditLoading(true);
    try {
  ...
  const handleDeleteImage = useCallback(async (imageId) => {
    ...
    try {
      await axiosInstance.delete(`/products/remove-image/${id}`, { data: { image_id: imageId } });
      await fetchProduct();
  ...
  const handleDeleteTag = useCallback(async (tagId) => {
    ...
    try {
      await axiosInstance.delete(`/products/remove-tags/${id}`, { data: { tag_ids: [tagId] } });
      await fetchProduct();
  ...
  const handleAddCategories = useCallback(async () => {
    ...
    try {
      await axiosInstance.post(`/products/add-categories/${id}`, { category_id: selectedCategories });
      await fetchProduct();
  ...
  const handleDeleteCategory = useCallback(async (categoryId) => {
    ...
    try {
      await axiosInstance.delete(`/products/remove-categories/${id}`, { data: { category_id: [categoryId] } });
      await fetchProduct();
  ...
  const handleDeleteBundleItem = useCallback(async (bundleId) => {
    if (!window.confirm("Are you sure you want to remove this item from the bundle?")) return;
    ...
    try {
      ...
      toast.error(err.response?.data?.message || "Failed to delete bundle item");
    } finally {
  ...
  const handleSelectProduct = useCallback((option) => {
    if (option && !selectedItems.some(item => item.id === option.value)) {
      setSelectedItems([...selectedItems, { id: option.value, name: option.label, stock: option.stock, image: option.image }]);
  ...
  const handleAddToBundle = useCallback(async () => {
    ...
    try {
      ...
      setBundleError(err.response?.data?.message || "Failed to add bundle items");
    } finally {
      setBundleLoading(false);
    }
  }, [id, selectedItems, bundleQuantity, fetchProduct]);

  const handleUpdateBundleQuantity = useCallback(async (bundleId, newQuantity) => {
    ...
    try {
      await axiosInstance.post(`/products/bundles/update-quantity/${bundleId}`, { quantity: parseInt(newQuantity) });
      await fetchProduct();
      setEditingQuantity({});
  ...
  const handleBundleToggle = useCallback(async () => {
    ...
    try {
        await axiosInstance.post(`/products/bundles/toggle-bundle/${id}`, { is_bundle: product.is_bundle === 1 ? 0 : 1 });
        setProduct(prev => ({ ...prev, is_bundle: prev.is_bundle === 1 ? 0 : 1 }));
        toast.success(`Product ${product.is_bundle === 1 ? "unmarked" : "marked"} as bundle successfully`);
    } catch (err) {
        toast.error(err.response?.data?.message || "Failed to toggle bundle status");
    } finally {
      ...
      <div className="mt-4">
        <Row>
            <Col lg={7}>
                <ProductInformation
                    editForm={editForm}
                    handleEditFormChange={(e) => setEditForm(prev => ({ ...prev, [e.target.name]: e.target.value }))}
                    handleEditorChange={(newContent) => setEditForm(prev => ({ ...prev, description: newContent }))}
        ...
        <Row>
            <Col>
                <ProductBundle
                    ...
                    handleRemoveSelectedItem={(itemId) => setSelectedItems(prev => prev.filter(item => item.id !== itemId))}
